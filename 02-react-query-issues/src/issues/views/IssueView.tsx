...
export const IssueView = () => {
  ...
  const { issueData, commentData } = useIssue(+id);

  if (issueData.isLoading) return <Loader />;
  if (!issueData.data) return <Navigate to={"/issues/list"} />;
  ...
  return (
    <div className="mb-5 row">
      ...
      {/* Primer comentario */}
      <IssueComment issueData={issueData.data} />

      {/* Comentario de otros */}
      {commentData.isLoading && <Loader />}
      {commentData.data?.map((issue) => (
        <IssueComment key={issue.id} issueData={issue} />
      ))}
